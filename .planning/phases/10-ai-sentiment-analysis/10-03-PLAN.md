...
<task type="auto">
  <name>Task 2: Strona analyze (replace mock) + keywords w settings</name>
  <files>
    src/app/(hub)/fb-analyzer/analyze/page.tsx
    src/components/fb/SettingsForm.tsx
  </files>
...
2. **SettingsForm.tsx** — Dodaj sekcje "Slowa kluczowe" (4. karta/sekcja):
   ...
   - Load: w GET /api/fb-settings dodaj obsluge `fb_keywords` — sprawdz czy fb-settings GET juz zwraca fb_keywords. Jesli NIE:
     * Dodaj osobny fetch: GET /api/fb-settings i w komponencie parsuj. LUB
     * Dodaj do GET response w fb-settings/route.ts nowe pole `fb_keywords: string[]` (parsowany JSON z value_plain klucza 'fb_keywords')
     * PREFEROWANE: Dodaj `fb_keywords` do GET response w fb-settings/route.ts
   ...
   WAZNE: Jesli fb-settings GET route nie zwraca jeszcze fb_keywords, ROZSZERZ GET response:
   ```typescript
