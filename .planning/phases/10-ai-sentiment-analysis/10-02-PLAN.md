...
must_haves:
  truths:
    - "POST /api/fb/analysis tworzy fb_analysis_jobs row z metadata { forceReanalyze } i zwraca jobId + totalPosts"
    ...
    - "forceReanalyze jest odczytywany z job.metadata (nie z request body) w process route"
  ...
  key_links:
    ...
    - from: "src/app/api/fb/analysis/route.ts"
      ...
      via: "insert job row with metadata"
      pattern: "fb_analysis_jobs.*insert.*metadata"
    - from: "src/app/api/fb/analysis/process/route.ts"
      to: "fb_analysis_jobs.metadata"
      via: "reads forceReanalyze from job.metadata"
      pattern: "metadata.*forceReanalyze"
---

<objective>
Stworzenie 3 API routes do analizy AI postow FB: tworzenie jobow (z metadata JSONB przechowujacym forceReanalyze), batch processing z structured JSON output i pause/resume. Kazdy post analizowany indywidualnie (1 AI call), 5 postow rownolegle per request, wyniki zapisywane bezposrednio na fb_posts.

Purpose: Te API routes sa core engine Phase 10 — bez nich frontend nie moze uruchomic analizy. Pattern identyczny jak email analyzer (polling-driven batch processing) ale prostszy (wyniki na fb_posts, nie w osobnej tabeli). forceReanalyze jest persisted w job.metadata (pattern identyczny jak sync_jobs.metadata w email-analyzer) zeby process route mogl go odczytac miedzy requestami pollingowymi.
...
8. INSERT fb_analysis_jobs: `{ group_id, status: 'pending', total_posts: count, analyzed_posts: 0, progress: 0, started_at: now(), metadata: { forceReanalyze: forceReanalyze || false } }`
   WAZNE: `metadata` JSONB przechowuje `forceReanalyze` — process route odczytuje go stamtad (NIE z request body). Pattern identyczny jak sync_jobs.metadata w email-analyzer.
...
6. Return: `{ jobs: [...] }`
  ...
  - `grep "metadata" src/app/api/fb/analysis/route.ts` potwierdza zapis forceReanalyze w metadata JSONB
  </verify>
  <done>
  - POST /api/fb/analysis tworzy job w fb_analysis_jobs z metadata { forceReanalyze }, waliduje grupe/AI config/brak aktywnego joba, zwraca jobId
  - GET /api/fb/analysis zwraca liste ostatnich jobow z group name
  - forceReanalyze persisted w job.metadata — dostepny dla process route bez przekazywania w request body
  </done>
</task>
...
4. Load job z fb_analysis_jobs (WAZNE: SELECT musi zawierac kolumne `metadata`). Jesli completed/failed — return status. Jesli paused — return paused + hasMore: false.
5. Jesli status = 'pending' — update na 'running'
6. **Odczytaj forceReanalyze z job.metadata:**
   ```typescript
   const forceReanalyze = (job.metadata as Record<string, unknown>)?.forceReanalyze === true;
   ```
   NIE odczytuj forceReanalyze z request body — jest persisted w job.metadata (pattern identyczny jak sync_jobs.metadata).
7. Load AI config: `loadAIConfig(adminClient)`
8. Load group z fb_groups (potrzebne: name, developer, ai_instruction)
9. Resolve prompt:
...
10. Load keywords: `loadKeywords(adminClient, job.group_id)`
11. Get next batch unanalyzed posts:
    - Jesli `forceReanalyze` (odczytany z job.metadata): SELECT WHERE group_id AND content IS NOT NULL, uzyj offsetu opartego na analyzed_posts (OFFSET job.analyzed_posts, LIMIT POSTS_PER_REQUEST)
...
12. Pre-filter: posty z content.length < MIN_CONTENT_LENGTH -> oznacz jako nieistotne:
    ```
...
13. Procesuj pozostale posty rownolegle via `Promise.allSettled`:
    ```typescript
...
15. Return: `{ status, analyzedPosts, totalPosts, hasMore }`
16. Error handling: catch na calym try block -> update job status = 'failed' + error_message
...
- forceReanalyze ZAWSZE odczytywany z job.metadata, NIGDY z request body w process route
  </action>
  <verify>
  - `npx tsc --noEmit` przechodzi
  ...
  - `grep "metadata" src/app/api/fb/analysis/process/route.ts` potwierdza odczyt forceReanalyze z job.metadata
  - `grep -E "'pause'|'resume'|'cancel'" src/app/api/fb/analysis/pause/route.ts` potwierdza obsluge 3 akcji
  </verify>
  <done>
  ...
  - POST /api/fb/analysis/process odczytuje forceReanalyze z job.metadata (persisted w DB, nie z request body)
  ...
  - Posty z sentiment IS NOT NULL sa pomijane (chyba ze forceReanalyze w job.metadata)
...
- process route odczytuje forceReanalyze z job.metadata (nie z request body)
- analysis route zapisuje forceReanalyze w metadata JSONB przy INSERT
...
- forceReanalyze jest persisted w job.metadata i odczytywany w process route — dziala poprawnie miedzy requestami pollingowymi
