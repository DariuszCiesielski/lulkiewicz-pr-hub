...
<task type="auto">
  ...
  <action>
    1. Utworz tabele w Supabase przez Management API (Supabase CLI broken — patrz STATE.md).
       ...
       -- KROK 1: Zarejestruj konto admina przez Supabase Auth (signup email+haslo)
       --         lub uzyj supabase.auth.admin.createUser z service role key
       -- KROK 2: Pobierz UUID nowo utworzonego auth usera
       -- KROK 3: Wykonaj INSERT:
       INSERT INTO app_allowed_users (email, user_id, role, display_name)
       VALUES ('EMAIL_ADMINA', 'USER_UUID_Z_KROKU_2', 'admin', 'Administrator');
       -- ZASTAP 'EMAIL_ADMINA' i 'USER_UUID_Z_KROKU_2' rzeczywistymi wartosciami!
       -- Zapytaj uzytkownika o email admina jesli nie jest znany.
       -- Bez tego INSERT nikt nie ma roli admin i panel admina nie bedzie dostepny.
       ```
       ...
       UWAGA: Projekt Supabase MUSI juz istniec (utworzony w planie 01-01).
       Ref i klucze powinny byc w .env.local — zweryfikuj to przed wykonaniem SQL.
  ...
  <verify>
    - Supabase credentials w .env.local sa poprawne: `curl https://{ref}.supabase.co/rest/v1/ -H "apikey: {ANON_KEY}"` zwraca 200
    - SQL wykonany bez bledow (sprawdz odpowiedz API)
    - `SELECT * FROM app_allowed_users;` zwraca rekord pierwszego admina (email, role='admin')
    - `SELECT public.is_admin();` dziala (zwraca false dla nowego usera, true dla admina po zalogowaniu)
    - Pierwszy admin moze sie zalogowac na stronie /login i po zalogowaniu ma role admin w AuthContext
    - `npm run build` przechodzi z zaktualizowanym AuthContext
  </verify>
  <done>
    Tabela app_allowed_users istnieje z RLS, is_admin() SECURITY DEFINER dziala, pierwszy admin INSERT wykonany, AuthContext pobiera role i allowed_tools po zalogowaniu.
  </done>
</task>
