...
<task type="auto">
  ...
  <action>
    1. Utworz tabele w Supabase przez Management API (Supabase CLI broken — patrz STATE.md).
       ...
       ```bash
       curl -X POST "https://api.supabase.com/v1/projects/vyiuzaazrfypqzilptnb/database/query" \
         -H "Authorization: Bearer sbp_bc8b108c92b88684245186533762101587b8f1bd" \
         -H "Content-Type: application/json" \
         -d '{"query": "SQL_TUTAJ"}'
       ```

       UWAGA: Supabase project ref to `vyiuzaazrfypqzilptnb`, access token to `sbp_bc8b108c92b88684245186533762101587b8f1bd` (z STATE.md Memory).

       WAZNE: Jesli projekt Supabase jeszcze nie istnieje (STATUS: TBD w STATE.md), NAJPIERW go utworz przez Management API z region `eu-central-1` (Frankfurt, GDPR). Sprawdz najpierw czy istnieje:
       ```bash
       curl -s "https://api.supabase.com/v1/projects/vyiuzaazrfypqzilptnb" \
         -H "Authorization: Bearer sbp_bc8b108c92b88684245186533762101587b8f1bd" | head -c 200
       ```
       Jesli nie istnieje, utworz nowy projekt:
       ```bash
