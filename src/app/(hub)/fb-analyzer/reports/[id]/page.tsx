...
import { ArrowLeft, Copy, Check, Edit3, Save, X, Download, List, Users } from 'lucide-react';
...
export default function FbReportDetailPage() {
  const { isAdmin, isLoading: authLoading } = useAuth();
  const router = useRouter();
  const params = useParams();
  const reportId = params.id as string;

  const [report, setReport] = useState<FbReport | null>(null);
  ...
  const handleSave = async () => {
    ...
    const res = await fetch(`/api/fb-reports/${reportId}`, {
      method: 'PATCH',
  ...
  const handleExportDocx = async () => {
    ...
    setExporting(true);
    try {
      // Map FB report fields to ReportMeta expected by export function
      const reportMeta = {
        title: report.title,
  ...
  if (!isAdmin || error || !report) {
    return (
      <div className="mx-auto max-w-4xl">
        <button
          onClick={() => router.push('/fb-analyzer/reports')}
  ...
  const dateRangeText =
    report.date_from && report.date_to
      ? `${new Date(report.date_from).toLocaleDateString('pl-PL')} — ${new Date(report.date_to).toLocaleDateString('pl-PL')}`
      : null;
      ...
      {sortedSections.length > 1 && (
        ...
        >
          ...
          >
            ...
            Spis treści ({sortedSections.length} sekcji)
          </button>
          {showToc && (
